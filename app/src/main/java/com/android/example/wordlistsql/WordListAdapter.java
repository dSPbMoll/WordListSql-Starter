...
public class WordListAdapter extends RecyclerView.Adapter<WordListAdapter.WordViewHolder> {
    ...
    public void onBindViewHolder(WordViewHolder holder, int position) {
        ...
        // Crear el listener con id y word
        MyButtonOnClickListener listener =
                new MyButtonOnClickListener(current.getId(), current.getWord());

        // Asociarlo al botón DELETE
        holder.delete_button.setOnClickListener(v -> {
            int pos = holder.getAdapterPosition();
            if (pos != RecyclerView.NO_POSITION) {

                WordItem item = mDB.query(pos);

                new AlertDialog.Builder(mContext)
                        .setTitle("Delete word")
                        .setMessage("Are you sure you want to delete \"" + item.getWord() + "\"?")
